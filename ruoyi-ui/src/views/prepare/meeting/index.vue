<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['prepare:meeting:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['prepare:meeting:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['prepare:meeting:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['prepare:meeting:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="meetingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="序号" align="center" prop="meetingId" v-if="columns[0].visible" />
      <el-table-column label="会议编号" align="center" prop="meetingCode" v-if="columns[1].visible" />
      ...
      <el-table-column label="会议主题" align="center" prop="meetingTopic" v-if="columns[4].visible" />
      <el-table-column label="会议时间" align="center" prop="meetingTime" width="180" v-if="columns[5].visible">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.meetingTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="会议地点" align="center" prop="meetingLocation" v-if="columns[6].visible" />
      <el-table-column label="会议时长" align="center" prop="meetingDuration" v-if="columns[7].visible" />
      <el-table-column label="会议文件" align="center" prop="meetingDocument" v-if="columns[8].visible">
        <template slot-scope="scope">
          <div v-if="scope.row.meetingDocument && scope.row.meetingDocument.length > 0">
            <div v-for="(file, index) in scope.row.meetingDocument.split(',')" :key="index" class="file-item">
      ...
      <el-table-column label="创建时间" align="center" prop="createdAt" width="180" v-if="columns[10].visible">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="会议主题" prop="meetingTopic">
          <el-input v-model="form.meetingTopic" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        ...
        <el-form-item label="会议时长" prop="meetingDuration">
          <el-input v-model="form.meetingDuration" placeholder="请输入会议时长" />
        </el-form-item>
        <el-form-item label="会议文件" prop="meetingDocument">
          <file-upload v-model="form.meetingDocument"/>
        </el-form-item>
...
<script>
import { listMeeting, getMeeting, delMeeting, addMeeting, updateMeeting } from "@/api/prepare/meeting";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 1, label: `会议编号`, visible: true },
        { key: 2, label: `项目名称`, visible: true },
        { key: 3, label: `承包商`, visible: true },
        { key: 4, label: `会议主题`, visible: true },
        { key: 5, label: `会议时间`, visible: true },
        { key: 6, label: `会议地点`, visible: true },
        { key: 7, label: `会议时长`, visible: true },
        { key: 8, label: `会议文件`, visible: true },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除协调会议编号为"' + meetingIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('prepare/meeting/export', {
        ...this.queryParams
