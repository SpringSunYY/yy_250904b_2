  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dangerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设备位号" :show-overflow-tooltip="true" align="center" v-if="columns[6].visible"
                       prop="equipCode"
      />
      <el-table-column label="有无隐患" align="center" v-if="columns[7].visible" prop="isDanger">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.equip_has_danger" :value="scope.row.isDanger"/>
        </template>
      </el-table-column>
      <el-table-column label="隐患等级" align="center" v-if="columns[8].visible" prop="dangerLevel">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.equip_danger_level" :value="scope.row.dangerLevel"/>
        </template>
      </el-table-column>
      <el-table-column label="隐患描述" :show-overflow-tooltip="true" align="center" v-if="columns[9].visible"
                       prop="description"
      />
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 8, label: '隐患等级', visible: true },
        { key: 9, label: '隐患描述', visible: true },
